...
$iftheni.data "%database%"=="KTBL_database"
   $$GDXIN '%datDir%/cropop_ktbl.gdx'
      $$LOAD set_machType=machine set_machTypeID=machineID set_machTypeID_machType=machineID_machine machineType set_machType_machineType=machine_machineType
   $$GDXIN
$endif.data
*
...
*
* --- load machine attributes of machines required for selected crops
*
   parameter p_machAttr(machType,machAttr)
...
*  (VarCost_x correspond to repairCosts, not depreciation. Why is it deleted?)
