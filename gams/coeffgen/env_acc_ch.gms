...
p_CfIntensTill(m,crops) = sum((labPeriod_to_month(labPeriod,m),intensOperation(operation),till,intens),p_crop_op_per_till(crops,intensOperation,labPeriod,till,intens));
...
$$iftheni.up "%upstreamEF%" == "true"
p_EFInput("soyBeanMealEU","CO2_eq") = p_EFInput("soyBeanMealEU","CO2_eq")   + p_EFInput("soyBeanMealEU","CO2_eq")  * %redPotConcEU%;
p_EFInput("soyBeanMeal","CO2_eq") = p_EFInput("soyBeanMeal","CO2_eq") + p_EFInput("soyBeanMeal","CO2_eq") * %redPotConcIMM%;
p_EFInput("soyBeanMealBRA","CO2_eq") = p_EFInput("soyBeanMealBRA","CO2_eq") + p_EFInput("soyBeanMealBRA","CO2_eq") * %redPotConcBRA%  ;

p_EFInput("ConcCattle1","CO2_eq") =   0.6555 * p_EFInput("WinterBarley","CO2_eq") + 0.07 * p_EFInput("WinterBarley","CO2_eq") 
                                  + 0.2285 * p_EFInput("MaizCCM","CO2_eq") + 0.0188 * p_EFInput("oilsForFeed","CO2_eq");

p_EFInput("ConcCattle2","CO2_eq") =   0.08 *  p_EFInput("WinterWheat","CO2_eq") + 0.5085 * p_EFInput("WinterBarley","CO2_eq") 
                                  + 0.2893 *p_EFInput("rapeseedMeal","CO2_eq") + 0.2893 *p_EFInput("oilsForFeed","CO2_eq") 
                                  + 0.1007 *p_EFInput("maizCCM","CO2_eq");

p_EFInput("ConcCattle3","CO2_eq")   =   0.4298 * p_EFInput("soyBeanMeal","CO2_eq") + 0.5698 * p_EFInput("rapeseedmeal","CO2_eq");
p_EFInput("ConcCattle3EU","CO2_eq") =   0.4298 * p_EFInput("soyBeanMealEU","CO2_eq")  + 0.5698 * p_EFInput("rapeseedmeal","CO2_eq");
p_EFInput("ConcCattle3BRA","CO2_eq") =   0.4298 * p_EFInput("soyBeanMealBRA","CO2_eq")  + 0.5698 * p_EFInput("rapeseedmeal","CO2_eq");
$endif.up
*
...
p_EFoperations(crops,plot,till,intens,operation,emissions,t)$(p_EFoperations(crops,plot,till,intens,operation,emissions,t)=0)
    =   sum( (actMachVar,actPlotSize), op_attr(operation,actMachVar,actPlotSize,"diesel")) *  9.79 * 3.6 * operationsemissions("dieselBurned",emissions);

* kill unwanted combinations
p_EFoperations(crops,plot,till,intens,operation,emissions,t) =
p_EFoperations(crops,plot,till,intens,operation,emissions,t) $(c_p_t_i(crops,plot,till,intens)$sum(labPeriod,crop_op_per_till(crops,operation,labperiod,till,intens))$(not sameas ("manDist",operation)))
 + (operationsemissions("mandist",emissions) $(c_p_t_i(crops,plot,till,intens) $sameas(operation,"mandist")))
;
display inputsEmissions;
$$else.ecoInvent

     option kill=p_EFOperations;
     option kill=p_EFInputCrops;

* ---- for fertilizers EF have to be converted from kg N to kg fertilizer
   p_EFInput("AHL","CO2") = p_EFInput("AHL","CO2") * 0.32;
   p_EFInput("ASS","CO2") = p_EFInput("ASS","CO2") * 0.27;;
   p_EFInput("PK_18_10",emissions) = p_EFInput("PK_18_10",emissions) * 0.18;
   p_EFInput("KAS",emissions) = p_EFInput("KAS",emissions) * 0.76;
   p_EFInput("KaliMag",emissions) = p_EFInput("Kalimag",emissions) * 0.3;

*  --- lime and water
   p_EFInputCrops(crops,till,intens,"lime",emissions) $sum(plot$c_p_t_i(crops,plot,till,intens),p_costQuant(crops,till,intens,"lime"))
       = p_EFInput("lime",emissions)*1000;
...
$$endif.ecoInvent
*add manure disposal for all combinations at is not standard for most crops in crop_op_per_till
